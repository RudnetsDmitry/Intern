...
void Critical_Path (int i[], int j[], int dij[],
...
{
    constexpr int n1=9;    //количество вершин в графе
    int a[n1][n1]; // матрица связей
    int v[n1]; // посещенные вершины
    int temp=0;
    int minindex, min;
...
namespace gantt_chart
{
    class GantChartWindow : public QMainWindow
    {
        using base_t = QMainWindow;
    public:
        GantChartWindow(QWidget * centerWidget)
        {
    ...
    QMainWindow * MakeChart()
    {
		...
		Critical_Path(&i[0], &j[0], &dij[0], &s1[0], &s2[0], &f1[0], &f2[0], &tf[0], &ff[0]);
		...
		auto * form = new Form(&s1[0], &f1[0], &s2[0], &f2[0], &ff[0], &tf[0], &i[0], &j[0], s, &mas[0], t, d[8]);
        return new GantChartWindow(form);
    }
}
